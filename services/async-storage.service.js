...
function getByEmail(entityType, entityEmail) {
    return query(entityType).then(entities => {
        const entity = entities.find(entity => entity.email === entityEmail)
        if (!entity) throw new Error(`Get failed, cannot find entity with email: ${entityEmail} in: ${entityType}`)
        return entity
    })
}
